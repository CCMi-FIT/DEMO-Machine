...
data Cookie = Cookie
  { cName :: String
  , cVal :: String
  , mbCExpires :: Maybe Int
  , mbCPath :: Maybe String
  , mbCDomain :: Maybe String
  , mbCSecure :: Maybe Bool
  }

mkCookie :: String -> String -> Cookie
mkCookie name val = Cookie name val Nothing Nothing Nothing Nothing

setCExpires :: Int -> Cookie -> Cookie
setCExpires expires cookie = cookie { mbCExpires = Just expires }

setCPath :: String -> Cookie -> Cookie
setCPath path cookie = cookie { mbCPath = Just path }

setCDomain :: String -> Cookie -> Cookie
setCDomain domain cookie = cookie { mbCDomain = Just domain }

setCSecure :: Bool -> Cookie -> Cookie
setCSecure secure cookie = cookie { mbCSecure = Just secure }

setCookie :: Cookie -> IO ()
setCookie cookie =
  let
    nameJs = toJSString $ cName cookie
    valJs = toJSString $ cVal cookie
    params = toObject
        [ ("expires" :: JSString, toAny $ mbCExpires cookie)
        , ("path", toAny $ mbCPath cookie)
        , ("domain", toAny $ mbCDomain cookie)
        , ("secure", toAny $ mbCSecure cookie)
        ]
  in doFFI nameJs valJs params
    where
    doFFI :: JSString -> JSString -> JSAny -> IO ()
    doFFI = ffi "(function (name, val, params) { Cookies.set(name, val, params); })"

getCookie :: String -> IO (Maybe String)
getCookie name = let nameJs = toJSString name in doFFI nameJs
  where
  doFFI :: JSString -> IO (Maybe String)
  doFFI = ffi "(function (name) { return Cookies.get(name); })"

removeCookie :: String -> IO (Maybe String)
removeCookie name = let nameJs = toJSString name in doFFI nameJs
  where
  doFFI :: JSString -> IO (Maybe String)
  doFFI = ffi "(function (name) { return Cookies.remove(name); })"
